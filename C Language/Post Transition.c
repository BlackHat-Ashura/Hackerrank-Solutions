...
void send_all_acceptable_packages(town source, int source_office_index, town target, int target_office_index) {
    ...
    package* target_packages = (package *)malloc(sizeof(package) * (target.offices[target_office_index].packages_count + n));
    package* source_packages = (package *)malloc(sizeof(package) * (source.offices[source_office_index].packages_count - n));

    for (int i = 0; i < target.offices[target_office_index].packages_count; i++) {
        target_packages[i] = target.offices[target_office_index].packages[i];
    }
    ...
    for(int i = 0; i < source.offices[source_office_index].packages_count; i++) {
        if(source.offices[source_office_index].packages[i].weight >= target.offices[target_office_index].min_weight &&
           source.offices[source_office_index].packages[i].weight <= target.offices[target_office_index].max_weight) {
            target_packages[n] = source.offices[source_office_index].packages[i];
            n++;
        }
        else {
            source_packages[m] = source.offices[source_office_index].packages[i];
            m++;
        }
    }
    ...
    target.offices[target_office_index].packages = target_packages;
    source.offices[source_office_index].packages = source_packages;
}
